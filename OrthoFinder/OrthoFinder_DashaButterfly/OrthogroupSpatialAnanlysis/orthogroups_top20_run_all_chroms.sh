...
echo "chrom "$(awk 'ORS=NR%83?" ":RS' orthogroup.top20.ref.list)" "
...
do
	...
	do
		...
		bash orthogroup_counter_20topOG.sh $line $1 > $line.orthogr.top20.count.out
		echo "$line "$(awk '{print $2}' $line.orthogr.top20.count.out | awk 'ORS=NR%83?" ":RS' )" "
